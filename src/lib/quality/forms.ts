...
export const QA_FORM_SEED: QaForm[] = [
  ...
  },
  {
     ...
     * beneath one of those without its own score is part of it — the pair or
     * the run forfeits those points once, never once each (see subItems).
     *
     * Audits filed against an earlier version keep their own scores and their
     * own labels: qa_audits stores the figures, qa_audit_results stores the
     * category and attribute as text. Nothing already filed moves when this
     * changes, but a Fax trend spanning a change compares two rules.
     */
    definition: {
      type: "B",
      groups: [
        {
          name: "Provider Information",
          items: [
            { label: "Agent selected Incorrect Provider", points: 5, subItems: ["Incorrect NPI was entered"] },
        ...
        },
        {
          ...
          items: [
            ...
            { label: "Agent selected Correct Strength", points: 4 },
            {
              ...
              points: 4,
              subItems: ["Quantity and days supply must be entered exactly according to the attachment"],
            },
            { label: "Brand vs Generic Accuracy", points: 3 },
            {
              label: "Agent selected Correct TCE radio button",
              points: 3,
              subItems: ["Ensure the Pre-Benefit Determination flag is selected appropriately when applicable"],
            },
          ...
          items: [
            {
              ...
              points: 5,
              subItems: ["Agent must change the NDC if an invalid drug was previously selected"],
            },
            {
              label: "Agent failed to use Retry Trial Claim function",
              points: 5,
              subItems: ["Retry Trial claim was used for RAFA and RARA cancellation"],
            },
            {
              label: "Agent initiate PA and any new PA’s associated with other medication listed in the fax request",
              points: 5,
            },
            { label: "Agent bypass dup screen/forward to appeals/reworks/merge correctly", points: 5 },
            { label: "Agent cancelled the case properly", points: 5, subItems: ["Correct cancel reason code was used"] },
            { label: "Agent Reworked when necessary", points: 5 },
            {
              label: "Agent correctly selected Multiple Review PA Type",
              points: 5,
              subItems: ["Agent must identify and add any missing PA Type reviews to the Multiple Reviews dropdown"],
            },
          ],
        },
        {
          ...
          items: [
            ...
            { label: "Tried and Failed medications", points: 5 },
          ],
        },
        {
          name: "Documentation",
      ...
      compliance: ["Wrong Member Selected", "Fax Priority", "Invalid Cancellation"],
    },
  },
];
