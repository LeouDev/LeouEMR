...
-- on the new table (what re-running scripts/sql/app-role.sql would do), and
...
-- run twice: every statement is guarded, and both the seed and the tracker
-- insert are too.
--
...
-- empty and the rest of the action item page — the RCA, the plan's prose,
-- the dates, the acknowledgement — is untouched. The code is deployed
-- first; this is what fills the list.
...
-- deploy; `active = false` retires one without breaking the plans that
-- already point at it.
...
do $$
begin
  if not exists (select 1 from pg_constraint where conname = 'action_plan_categories_code_unique') then
    alter table public.action_plan_categories add constraint action_plan_categories_code_unique unique (code);
  end if;
end $$;

-- Nullable on purpose. Every plan written before this list existed has
-- prose and no category; classifying those after the fact would mean
-- inventing a choice nobody made. The form requires one for anything new.
alter table public.action_plans add column if not exists category_id uuid;

do $$
begin
  if not exists (
    select 1 from pg_constraint where conname = 'action_plans_category_id_action_plan_categories_id_fk'
  ) then
...
-- its own so it is never the thing a tired reader lands on by accident.
...
-- Row level security, then the app role's policy — the same treatment
-- root_cause_categories has, being the same kind of reference list.
...
begin
  if not exists (
    ...
    where schemaname = 'public' and tablename = 'action_plan_categories' and policyname = 'emr_app_full_access'
  ) then
    create policy emr_app_full_access on public.action_plan_categories for all to emr_app using (true) with check (true);
  end if;
end $$;
...
select '3d7cbf6a8d574fa4deeb23e698a17c07fe005279be32b7e1f9866491b76fe0f3', 1790016906084 -- 0060_action_plan_categories
 where not exists (select 1 from drizzle.__drizzle_migrations where hash = '3d7cbf6a8d574fa4deeb23e698a17c07fe005279be32b7e1f9866491b76fe0f3');

commit;

-- Verify: expect 1, 1, 1, 17 and 3 groups.
select
